...
var PTF_DEFAULTS=[
  ...
  {id:"eth",symbol:"ETH",name:"Ethereum",geckoId:"ethereum",amount:1.531305,avgEntry:2344.42,totalCost:3590.02,source:"ledger",decimals:6,contract:null}
];
var PTF_VERSION=5;
...
function ptfLoad(){try{
  ...
  try{
    ...
    // One-time: ETH-Bestand exakt auf Ledger-Wallet-Balance 1,53130508 (alles auf Ledger-Mainnet, verifiziert 25.07.2026)
    if(!localStorage.getItem("ptf_ethfix_v2")){
      for(var _ei=0;_ei<ptfAssets.length;_ei++){if(ptfAssets[_ei].id==="eth"){ptfAssets[_ei].amount=1.531305;ptfAssets[_ei].avgEntry=2344.42;ptfAssets[_ei].totalCost=3590.02;break;}}
      ...
      localStorage.setItem("ptf_ethfix_v1","1");localStorage.setItem("ptf_ethfix_v2","1");
      try{localStorage.setItem("ptf_assets",JSON.stringify(ptfAssets));}catch(e){}
      try{ptfSyncServer();}catch(e){}
      console.log("PTF: ETH-Bestand auf 1,531305 korrigiert (Ledger-Balance, on-chain verifiziert)");
    }
  }catch(e){}
